...
tinput_data <- subset_expt(t_clinical, subset="batch!='biopsy'")
tc_norm <- normalize_expt(input_data, transform = "log2", convert = "cpm")
texprs <- t(exprs(tc_norm))
t_norm <- normalize_expt(tinput_data, transform = "log2", convert = "cpm")
ttexprs <- t(exprs(t_norm))

ref_col <- "finaloutcome"
outcome_factor <- as.factor(as.character(pData(input_data)[[ref_col]]))
comparison_n <- 200
toutcome_factor <- as.factor(as.character(pData(tinput_data)[[ref_col]]))
```
...
system.time({
  ...
  tequivalent <- normalize_expt(t_norm, filter = "cv", cv_min = 0.1)
})
...
tnzv_texprs <- t(exprs(tequivalent))
```
...
tnzv_center <- preProcess(tnzv_texprs, method = "center")
tnzv_texprs <- predict(tnzv_center, tnzv_texprs)
```
...
system.time({
  ...
  tnzv_correlated <- preProcess(tnzv_texprs, method = "corr", cutoff = 0.95)
  tnzv_uncorr <- predict(tnzv_correlated, nzv_texprs)
})
dim(nzv_uncorr)
```
...
tinteresting_meta <- pData(tinput_data)[, c("finaloutcome", "donor", "persistence",
                                            "visitnumber", "selectionmethod",
                                            "typeofcells", "time", "clinic")]


ml_df <- as.data.frame(cbind(outcome_factor, as.data.frame(nzv_uncorr)))
tml_df <- as.data.frame(cbind(outcome_factor, as.data.frame(tnzv_uncorr)))
ml_df[["outcome_factor"]] <- as.factor(ml_df[["outcome_factor"]])
tml_df[["outcome_factor"]] <- as.factor(tml_df[["outcome_factor"]])
dim(ml_df)
```
...
tdatasets <- create_partitions(tnzv_uncorr, tinteresting_meta,
                              outcome_factor = outcome_factor)
```
