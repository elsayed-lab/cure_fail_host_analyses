...
t_sex
t_sex_de <- all_pairwise(t_sex, model_batch = "svaseq",
                         parallel = parallel, filter = TRUE)
t_sex_de
...
pp(file = "images/compare_cf_and_visit_in_model_monocyte_logfc.svg")
logfc_plot
dev.off()
logfc_plot
...
logfc_plot <- logfc_plotter[["scatter"]] +
  ...
  ylab("DESeq2 log2FC: Default pairwise comparison") +
  ggtitle(glue("Comparing results from models: {prettyNum(cor_value[['estimate']])} (pearson)"))
pp(file = "images/compare_cf_and_visit_in_model_neutrophil_logfc.svg")
logfc_plot
dev.off()
logfc_plot
...
logfc_plot <- logfc_plotter[["scatter"]] +
  ...
  ylab("DESeq2 log2FC: Default pairwise comparison") +
  ggtitle(glue("Comparing results from models: {prettyNum(cor_value[['estimate']])} (pearson)"))
pp(file = "images/compare_cf_and_visit_in_model_eosinophil_logfc.svg")
logfc_plot
dev.off()
logfc_plot
...
```{r}
persistence_de_sva <- all_pairwise(persistence_expt, filter = TRUE,
...
```{r}
t_visit_all_de_sva <- all_pairwise(t_visit, filter = TRUE,
                                   parallel = parallel, model_batch = "svaseq")
t_visit_all_de_sva
...
external_de <- all_pairwise(external_cf, filter = TRUE,
                            parallel = parallel, model_batch = "svaseq")
external_de
...
```{r}
external_cf[["accession"]] <- rownames(pData(external_cf))
...
external_disease <- set_expt_conditions(external_cf, factor = "disease")
