...
is a composite of her tmrc3_ml and tmrc_sample_estimation notebooks.
In both cases, she loaded the initial data in a fashion nearly
identically to how I am in these worksheets, as a result I strongly
suspect that I can simply copy/paste her blocks of text and code, run
them, and get nearly identical results.  There is a decent chance that
I will thus scab some of the ideas and stick them into hpgltools so
that I may use them a bit more easily in other analyses.  I will make
notes for anything which I repurpose in this fashion; Theresa is
already an author on hpgltools, so I presume this is not a problem,
but just in case I will text her tomorrow and ask.

In her notebook, she started off by plotting essentially the same PCA
that I did in my 02sample_estimation worksheet in order to ensure that
we were working from similar/identical starting points.
...
```{r, eval=FALSE}
traits <- readxl::read_xlsx("../data/sample_sheets/tmrc3_demographicsv2.xlsx")
```
...
```{r, eval=FALSE}
lesion_columns <- c("codigo_paciente", "ef_lc_estado_final_estudio", "eb_lc_lesion_area_1",
                    "v2_lc_lesion_area_1",  "v3_lc_lesion_area_1", "v4_lc_lesion_area_1",
                    "v5_lc_lesion_area_1")
...
```{r}
ulcer_columns <- c("codigo_paciente","ef_lc_estado_final_estudio", "eb_lc_ulcera_area_1",
                   "v2_lc_ulcera_area_1", "v3_lc_ulcera_area_1",
                   "v4_lc_ulcera_area_1", "v5_lc_ulcera_area_1")
...
```{r}
interesting_traits <- traits[,c("eb_lc_sexo",
                                "eb_lc_etnia",
                                "edad",
                                "eb_lc_peso",
                                "eb_lc_estatura",
...
colnames(interesting_traits) <- c("Sex",
                                  "Ethnicity",
                                  "Age",
                                  "Weight",
                                  "Height",
...
interesting_traits_CF <- interesting_traits %>%
    subset(Final_Status != 3)
```
...
(and hopefully that extends to expression profiles?) can help us
...
pruned; so they are unlikely to be included in my version of this.

```{r}
table(interesting_traits_CF$Final_Status)
```

When we subset, we have 14 cures and 10 fail to cures, 3 lost.

```{r}
corr_cross(interesting_traits_CF, # name of dataset
           max_pvalue = 0.05, # display only significant correlations (at 5% level)
           top = 20 # display top 10 couples of variables (by correlation coefficient)
           )
```
...
interesting_traits_CFO <- interesting_traits_CF %>%
  subset(Final_Status != 2)

corr_var(interesting_traits_CFO, # name of dataset
         Final_Status, # name of variable to focus on
...
The gini index is a measure of how “pure” or homogeneous a population
is. So for example, if we had 2 populations consisting of both men and
women, but the proportions for each is as follows: group 1: 98% men,
2% women group2: 50% men, 50% women,
...
```{r}
v1_mono_table <- file.path(xlsx_prefix, "Monocytes", glue("t_monocyte_cf_tables_sva-v{ver}.xlsx"))
v1_mono_de <- readxl::read_xlsx(v1_mono_table, sheet = sheet_name, skip = 1)

v1_eo_table <- file.path(xlsx_prefix, "Eosinophils", glue("t_eosinophil_cf_tables_sva-v{ver}.xlsx"))
v1_eos_de <- readxl::read_xlsx(v1_eo_table, sheet = sheet_name, skip = 1)

v1_neut_table <- file.path(xlsx_prefix, "Neutrophils", glue("t_neutrophil_cf_tables_sva-v{ver}.xlsx"))
...
```{r}
p1 <- ggplot(hs_cart_2, aes(x = finaloutcome, y = ENSG00000182162, color = typeofcells)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point() +
  geom_hline(yintercept = 7.030467, color = "red") +
  theme_classic()

p2 <- ggplot(hs_cart_2, aes(x = finaloutcome, y = ENSG00000182162, color = typeofcells)) +
  geom_point() +
  geom_hline(yintercept = 7.030467, color = "red") +
  theme_classic()
...
## [1] "ENSG00000149418" "ENSG00000073737" "ENSG00000196126"
...
the samples which have tmm normalized expression of ENSG00000182162 >=
5.7

```{r}
hs_cart_2 %>%
  filter(ENSG00000182162  >= 5.7) %>%
  ggplot(aes(x = finaloutcome, y = ENSG00000203907, color = typeofcells)) +
  geom_boxplot(outlier.shape = NA) +
  geom_point() +
...
```{r}
vars_formula <- as.formula(paste0("finaloutcome ~ ", paste0(colnames(hs_int_fac)[c(-1)], collapse = " + ", recycle0 = FALSE)))
...
The following is similarly coming from Theresa's SVA_V3_Tumaco
document
