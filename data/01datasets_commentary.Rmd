...
The following block is not run in the container because I did not want
to time/spend the space installing the L.panamensis orgDB package.
The parasite data is sparse enough that it is not informative for
these analyses; but if you wish to prove me wrong, here is how I added
that information.
...
signify something is wrong.  This is why it is important to explicitly
set the year when downloading the ensembl gene annotations and make
certain that it matches the year of the genome's release.  Otherwise
those version numbers become increasingly likely to differ.
...
## The hopefully final changes to this document

I hope that this proves to be the final iteration of this document and
its various children.  The reason for its creation is to
change/improve the organization so that it exactly matches the flow
of the text in the manuscript; the reason the two documents do not
already match resides in the fact that this was created over the
various iterations of new data inclusion and so grew organically over
time and prioritized the readability and robustness of the code.  In
contrast, the manuscript was written with the primary constraint of
text readability.  Thus, the following few blocks should change
minimally, but will reorder the various filters; our assumption is
that it will result in identical data structures, the following will
...
explicitly checked/modified for mispeelings/capItilzation changes/etc.
'subset_genes()' is responsible for including _only_ the protein
coding genes in the data; it records the amount of information
...
to drop samples which are known to not be of interest, this includes
those which were our initial pilot study (PBMC samples), a separate
study which was initially included in the TMRC3 dataset (infected
macrophages), and the few samples from people who presented as healthy
in the clinic (when the clinicalpresentation column of the metadata is
'h').
...
A few tasks were immediately performed to simplify the data slightly.
Some important columns have minor but important elements which are not
completely standard; sanitize_expt_metadata() uses some heuristics to
fix them.  Some important columns were explicitly set to factors, and
a few samples which were used in pilot or external experiments were
removed via subset_expt().

# Repeat, but with combined host and parasite data

I recently re-quantified the data using salmon and a transcript
database comprised of both human and parasite sequences.  I would like
to look at this and see if it can aid us in understanding some
classification questions.

Note that without some changes to the annotations I cannot exclude the
features which are not of type 'protein_coding' because that would
remove all the panamensis annotations -- though I could just change
them above.

Given that making the correct gene->tx map is confusing, and made even
more so by the addition of a second species, I am going to first
create a tx-based expressionset and use the IDs from it to create the
gene version.
...
samples of interest and giving them hopefully useful names.
...
## Figure XX: Non-zero genes before sample filtering

```{r}
all_nz <- plot_nonzero(hs_expt)
pp(file = "figures/Sxx_nonzero_all_samples.svg")
all_nz$plot
dev.off()
all_nz
```
...
NOTE: I changed the prefix of the data structures to reflect the
clinic(s) from which they were acquired. This will likely be a problem
until I carefully step through the file; but will make sure I do not
accidently upload data with the Cali samples.

### The actual difference in this revision

The following block contains the actual changes which differentiate
this document from previous revisions.
...
Note that we now have two datasets which are comprised of only the
...
Note: I usually do this kind of thing in alphabetic order, but in this
instance I want to do monocytes (or neutrophils) first because they
have samples in all 4 of the clinic/cf categories.  The Eosinophils do
not have any Cali fails and this messes up the color scheme.  Though,
now that I have written this down, it would make much more sense to
just define these colors up above and use them across each set...
...
I added a few columns to the clinical gene annotations reflecting the
variance/stdev/mean expression of each gene.  This really highlights
the extraordinary degree to which genes are changing in the data...
